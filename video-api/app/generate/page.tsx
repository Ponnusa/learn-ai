...
function GenerateInner() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setLoading(true);
    try {
      const res = await generateVideo({ topic, subject: subject || undefined, language, aspect_ratio: aspectRatio }, token);
  ...
  return (
    <div className="max-w-xl mx-auto px-6 py-12">
      ...
      <form onSubmit={handleSubmit} className="flex flex-col gap-4">
        ...
        <div>
          <label className="text-sm font-medium block mb-1.5">Subject <span style={{ color: "var(--text-faint)" }}>(optional — auto-detected if omitted)</span></label>
          <input
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          <div>
            <label className="text-sm font-medium block mb-1.5">Aspect ratio</label>
            <select
              value={aspectRatio}
