...
async def _assert_course_access(course_id: str, teacher_id: str, db, cols: str = "id") -> dict:
    """Fetch a course the teacher owns. Raises 404 if not found."""
    row = await db.fetchrow(
        f"SELECT {cols} FROM courses c WHERE c.id = $1::uuid AND c.teacher_id = $2::uuid",
...
async def _src_concept_id(concept_id: str, db) -> str:
    """
    For shared-binary reads (concept_images, concept_resources):
    return source_concept_id if this is a teacher copy, else concept_id itself.
    """
    row = await db.fetchrow(
        "SELECT COALESCE(source_concept_id, id) AS eid FROM course_concepts WHERE id = $1::uuid",
        concept_id,
    )
    return str(row["eid"]) if row else concept_id
...
async def delete_course(course_id: str, authorization: str = Header(...)):
    ...
    async with get_db() as db:
        ...
        if row["status"] == "published":
            raise HTTPException(409, "Cannot delete a published course â€” archive it instead")
        await db.execute(
...
async def export_course(
    ...
    ?format=html   â†’ single self-contained HTML file (default)
    ?format=scorm  â†’ SCORM 1.2 ZIP importable into any LMS
    """
...
async def send_concept_chat_message(
    ...
    async with get_db() as db:
        ...
        src_cid = await _src_concept_id(concept_id, db)
...
async def apply_concept_chat_message(
    ...
    if req.action == 'block':
        ...
        async with get_db() as db:
            max_pos = await db.fetchval(
                "SELECT COALESCE(MAX(position), -1) FROM concept_content_blocks WHERE concept_id = $1::uuid AND section_id IS NULL",
                concept_id,
            )
            first_block = None
...
async def update_content_block(
    ...
    async with get_db() as db:
        await db.execute(
            ...
            f"WHERE id = $1::uuid AND concept_id = $2::uuid AND section_id IS NULL",
            *params,
        )
    return {"ok": True}
...
async def delete_content_block(
    ...
    async with get_db() as db:
        await db.execute(
            "DELETE FROM concept_content_blocks WHERE id = $1::uuid AND concept_id = $2::uuid AND section_id IS NULL",
            block_id, concept_id,
        )
    return {"ok": True}
...
async def reorder_content_blocks(
    ...
    async with get_db() as db:
        for item in items:
            await db.execute(
                "UPDATE concept_content_blocks SET position = $1 WHERE id = $2::uuid AND concept_id = $3::uuid AND section_id IS NULL",
...
# â”€â”€ Pipeline assets (AI-generated segment clips + images) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
async def get_pipeline_assets(concept_id: str, authorization: str = Header(...)):
    ...
    Joins generated_assets â†’ video_segments â†’ videos so only assets that were
    ...
    def _extract_r2_key(full_url: str | None) -> str | None:
        ...
        # pub-xxx.r2.dev/<r2_key> â€” no bucket segment
...
async def _generate_block_audio_bg(concept_id: str, block_id: str):
    ...
    try:
        ...
        response = await openai_client.audio.speech.create(
            model=resolve_model("tts-1"), voice="nova", input=script[:4096],
        )
...
async def get_course_curriculum_context(course_id: str):
    """
    Returns curriculum context for a course (public â€” UUID is scoped to the course).
                ...
                messages=[
                    {
                        ...
                        "content": (
                            ...
                            "Given a lesson's content, produce a single question title (8â€“14 words) that captures "
                            "what students will investigate â€” the kind of question a student would ask. "
...
class StudentChatRequest(BaseModel):
    ...
    direct:          bool = False       # True â†’ give direct answer (skip Socratic guidance)
    language:        str = 'en'
...
async def post_student_chat(
    ...
    # Fetch curriculum context first â€” it determines the entire prompt structure
    curriculum = None
    ...
    if curriculum and not req.direct:
        ...
        system_prompt = (
            ...
            f"Students discover science principles through hands-on experiments and discussion â€” NOT through direct instruction.\n\n"
            f"CRITICAL INSTRUCTION â€” INQUIRY PEDAGOGY:\n"
            ...
            f"  â€¢ \"What did you notice when you tried that in your experiment?\"\n"
            f"  â€¢ \"Why do you think one cup kept things colder than the other?\"\n"
            f"  â€¢ \"What do you think causes that to happen?\"\n"
            f"  â€¢ \"How does that connect to what you observed in class?\"\n"
            f"Use unit vocabulary naturally in your questions. Guide students toward the ideas â€” never lecture or give away the answer.\n"
        )
        ...
        system_prompt = (
            ...
            f"Give clear, direct, age-appropriate explanations. Be concise (3â€“5 sentences). "
        ...
        system_prompt = (
            ...
            f"Treat this as your only source of truth â€” never introduce facts beyond it, and "
            f"never recite it verbatim. Having it available does NOT mean you should explain it "
            f"directly by default: whether you answer directly or ask a guiding question first is "
            f"decided ONLY by the adaptive teaching style rules below, exactly as if this material "
            f"weren't here at all. A conceptual \"why does X happen\" or \"can X do Y\" question is "
            f"still a guided-discovery candidate even though the answer sits right here in this "
            f"material.\n\n{grounding[:10000]}{lang_note}\n\n"
            f"--- STAY ON TOPIC ---\n"
            f"This chat is scoped to \"{concept['title']}\". If the student asks about "
            f"something unrelated to it, gently redirect back to this concept's material "
...
async def _generate_concept_video_bg(concept_id: str, course_id: str, teacher_id: str | None = None, target_duration: int | None = None):
    ...
    try:
        async with get_db() as db:
            ...
            course = await db.fetchrow(
                "SELECT subject, grade, board, curriculum_context_id, language FROM courses WHERE id = $1::uuid", course_id
            )
        ...
        # â”€â”€ Build grade + TEKS context for the video prompt â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
            ...
            if cc and cc["teks_codes"]:
                ...
                if teks_descs:
                    ...
                    for code in cc["teks_codes"]:
                        ...
                        if desc:
                            extra_parts.append(f"  â€¢ {code}: {desc}")
                    extra_parts.append("Stay scoped to these standards â€” do not go beyond them.")
        extra = "\n\n".join(extra_parts)
        ...
        async with get_db() as db:
            video = await db.fetchrow("""
                ...
                VALUES ($1, $2, $3, '16:9', $4, 'pending')
                RETURNING id
            """, prompt, subject, course_lang, duration)
            video_id = video["id"]
            await db.execute(
        ...
        solution_data = await generate_solution_only(prompt, course_lang, duration)
        ...
        # â”€â”€ Storyboard branch (multimodal_video_enabled) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
        _multimodal = False
        if teacher_id:
            async with get_db() as db:
        ...
        if _multimodal:
            ...
            # Create content block + studio chat card (same as Phase 2 path below)
            if teacher_id:
                try:
                    async with get_db() as db:
                        max_pos = await db.fetchval(
                        ...
                        await db.execute("""
                            INSERT INTO messages (conversation_id, role, content, metadata)
                            VALUES ($1::uuid, 'assistant', '', $2::jsonb)
                        """, conv["id"], json.dumps({"content_type": "video", "block_id": block_id, "video_id": video_id}))
                except Exception as exc:
                    logger.error("[video] concept %s: storyboard chat card inject failed: %s", concept_id, exc)
            return
        ...
        code_data = await asyncio.wait_for(
            generate_manim_from_solution(solution_data, course_lang, duration, "16:9"),
            timeout=900,
        )
