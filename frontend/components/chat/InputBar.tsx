...
export function InputBar({
  ...
  return (
    <div className="px-4 pb-4 pt-2">
      ...
      {/* Bilingual active indicator — header bar handles picker, this just confirms active state */}
      {showBilingualControls && explanationLang && (
        <div className="mt-1.5 flex items-center justify-center">
          <div className="flex items-center gap-1.5 px-2.5 py-0.5 rounded-full bg-blue-500/8 border border-blue-500/20 text-[10px] text-blue-400/80">
            <span>{courseOpt?.flag} → {explainOpt?.flag}</span>
            <span>Bilingual mode on</span>
          </div>
        </div>
      )}
      ...
      {!micError && (
        <p className="text-center text-[var(--txa)] text-[10px] mt-1.5">
          {t.chat.disclaimer}
        </p>
      )}
    </div>
