...
interface PipelineSegment {
  ...
  source_image_url: string | null; source_image_r2_key: string | null;
...
export default function ConceptEditorPage() {
  ...
  async function handleImproveVideo(vidId: string, videoId: number, blockId: string) {
    ...
    if (!feedback) return;
    setImprovingVideoMsgs(prev => new Set([...prev, vidId]));
    try {
      const res = await fetch(`${API_BASE}/api/videos/${videoId}/improve`, {
        method: 'POST', headers: jsonH,
        body: JSON.stringify({ feedback }),
      });
      if (!res.ok) throw new Error('Improve failed');
      setVideoImprove(null);
      setChatMsgs(prev => prev.map(m =>
        m.id === vidId
          ? { ...m, videoStatus: 'pending', videoUrl: undefined, videoError: undefined }
          : m
      ));
      if (!videoPollingRef.current.has(blockId)) startVideoPolling(blockId, vidId);
    } catch { setVideoImprove(null); }
    finally {
      setImprovingVideoMsgs(prev => { const s = new Set(prev); s.delete(vidId); return s; });
    }
  }
                ...
                <div className="px-4 py-3 space-y-3">
                  {!chatLoaded ? (
                    ...
                    chatMsgs.map(m => {
                      ...
                      // ── Chat-triggered video cards: hidden here, rendered inline inside parent message ──
                      if (m.videoBlockId !== undefined && m.videoSourceMsgId) return null;

                      // ── Standalone video card (wand-generated, no source text message) ──
                      if (m.role === 'assistant' && m.videoBlockId && !m.videoSourceMsgId && !m.content) {
                        const vid = m;
                        ...
                        <div className="rounded-xl border border-[var(--bd)] overflow-hidden">
                          ...
                          <div className="divide-y divide-[var(--bd)]">
                            {run.segments.map(seg => {
                                ...
                                try {
                                  const r = await fetch(
                                    `${API_BASE}/api/courses/concepts/${conceptId}/pipeline-assets/add-to-textbook`,
                                    {
                                      ...
                                      headers: { ...authH, 'Content-Type': 'application/json' },
                                      body: JSON.stringify({
                                        r2_key:         seg.source_image_r2_key,
                                        narration_text: seg.narration_text,
                                        asset_type:     'image',
                                      }),
                                    },
                              ...
                              return (
                                <div key={seg.segment_id} className="p-3 flex flex-col gap-3">
                                  {/* Segment label */}
                                  <div className="flex items-center gap-2">
                                  ...
                                  {/* Source image — separate addable asset */}
                                      ...
                                      <div className="flex items-center justify-between px-2 py-1.5 bg-[var(--ov1)]">
                                        ...
                                        {addImageToTextbook && !imgAdded && (
                                          <button
                                            ...
                                            {isAddingImg ? <Loader2 size={9} className="animate-spin" /> : <Plus size={9} />}
                                            Add to Textbook
                                          </button>
                                        )}
                                  ...
                                  {/* Clip video — separate addable asset, middle-frame thumbnail */}
                                  {seg.clip_url && (
                                    <div className="rounded-lg overflow-hidden border border-[var(--bd)]">
                                      <video
                                        src={seg.clip_url}
