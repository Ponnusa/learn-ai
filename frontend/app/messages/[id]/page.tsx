    ...
    <div className="flex h-screen overflow-hidden bg-[var(--bg)]">
      ...
      <main className="flex-1 flex flex-col min-w-0 overflow-hidden">
        ...
        {loading ? (
          ...
          <div className="p-6 max-w-2xl mx-auto w-full flex flex-col flex-1 overflow-hidden">
            <button onClick={() => router.push('/messages')}
              className="flex items-center gap-1.5 text-[var(--tx7)] hover:text-[var(--purple)] text-sm mb-4 transition-colors shrink-0">
              <ArrowLeft size={15} /> {t.messages.backToMessages}
            </button>

            <div className="flex-1 overflow-y-auto space-y-2 mb-4 pr-1">
              {messages.length === 0 ? (
                <p className="text-[var(--tx7)] text-sm text-center py-12">{t.messages.noMessagesYet}</p>
              ) : messages.map(m => (
