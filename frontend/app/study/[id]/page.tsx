...
import {
  ArrowLeft, Upload, Loader, Brain, LayoutGrid, MessageSquare,
  ChevronRight, ChevronLeft, CheckCircle, RefreshCw,
  FileText, AlertCircle, Send, X, BookOpen,
} from 'lucide-react';
import { Sidebar } from '@/components/layout/Sidebar';
import { useSessionStore } from '@/store/sessionStore';
import {
  getStudySet, getStudySetStatus, uploadStudyMaterial,
  chatWithStudySet, reviewStudyCard,
  StudySetDetail, StudyConcept, StudyFlashcard,
} from '@/lib/api';
...
function ProcessingBanner({ status }: { status: string }) {
  ...
  const config = {
    empty:      { color: 'border-[var(--bd)] bg-[var(--ov2)]', icon: <Upload size={16} className="text-[var(--tx5)]" />, msg: 'Upload a PDF below to get started.' },
    processing: { color: 'border-yellow-500/30 bg-yellow-500/5',  icon: <Loader size={16} className="text-yellow-400 animate-spin" />, msg: 'Extracting concepts and generating flashcards — this takes about 30–60 seconds…' },
    failed:     { color: 'border-red-500/30 bg-red-500/5',        icon: <AlertCircle size={16} className="text-red-400" />,             msg: 'Processing failed. Try uploading again.' },
  }[status] ?? { color: 'border-[var(--bd)] bg-[var(--ov2)]', icon: null, msg: '' };

  return (
    <div className={`flex items-center gap-3 px-4 py-3 rounded-xl border mb-6 ${config.color}`}>
      {config.icon}
      <p className="text-[var(--tx4)] text-sm">{config.msg}</p>
    </div>
  );
}
    ...
    <div>
      <div
        ...
        className={`border-2 border-dashed rounded-2xl p-10 text-center transition-colors
          ${uploading ? 'border-indigo-500/40 bg-indigo-500/5' : 'border-[var(--bd)] hover:border-indigo-500/40 hover:bg-indigo-500/5 cursor-pointer'}`}
      >
        {uploading ? (
...
function OverviewTab({ ss, onRefresh }: { ss: StudySetDetail; onRefresh: () => void }) {
  return (
            ...
            {ss.concepts.map((c, i) => (
              <details key={c.id}
                ...
                <summary className="flex items-center gap-3 px-4 py-3 cursor-pointer list-none
                  ...
                  <ChevronRight size={14} className="text-[var(--tx6)] group-open/c:rotate-90 transition-transform" />
                </summary>
                <div className="px-4 pb-4 space-y-2 border-t border-[var(--bd)]">
                  <p className="text-[var(--tx3)] text-sm leading-relaxed pt-3">{c.definition}</p>
                  {c.explanation && (
                    <p className="text-[var(--tx5)] text-xs leading-relaxed">{c.explanation}</p>
                  )}
                </div>
              </details>
            ))}
  ...
  return (
    <div className="flex flex-col items-center gap-6 max-w-xl mx-auto">
      ...
      >
        ...
        <p className={`text-[var(--tx1)] leading-relaxed transition-opacity ${flipped ? 'text-base' : 'text-lg font-medium'}`}>
      ...
      {flipped ? (
        ...
        <button onClick={() => setFlipped(true)}
          className="px-8 py-2.5 rounded-xl bg-indigo-600 hover:bg-indigo-500
                     text-white text-sm font-medium transition-colors">
          Reveal answer
        </button>
      )}
...
function ChatTab({ ss }: { ss: StudySetDetail }) {
  ...
  async function send() {
    ...
    if (!text || loading || notReady) return;
    setInput('');
    ...
    try {
      ...
      const { reply } = await chatWithStudySet(ss.id, text, history, token ?? undefined);
    ...
    <div className="flex flex-col h-full min-h-[60vh]">
      ...
      <div className="flex-1 overflow-y-auto no-scrollbar space-y-4 pb-4">
        {messages.length === 0 && !notReady && (
          <div className="text-center py-12 text-[var(--tx6)] text-sm">
            <MessageSquare size={28} className="mx-auto mb-3 opacity-40" />
            Ask any question about "{ss.title}"
          </div>
        )}
        {messages.map((m, i) => (
...
export default function StudySetPage() {
  ...
  const load = useCallback(async () => {
    try {
      ...
      setSs(data);
      // If processing, switch to overview so user sees the progress banner
  ...
  useEffect(() => {
    load().then(status => {
      if (status === 'processing') startPoll();
    });
  ...
  function handleUploaded() {
    // Status flipped to 'processing' — reload and start polling
    load().then(() => startPoll());
  }
  ...
  if (!ss) {
    return (
      <div className="flex h-screen overflow-hidden bg-[var(--bg)]">
        <Sidebar onNewChat={() => router.push('/')} />
        <main className="flex-1 flex items-center justify-center text-[var(--tx5)]">
          Study set not found.
        </main>
      </div>
    );
      ...
      <main className="flex-1 flex flex-col min-w-0 overflow-hidden">
        ...
        <div className="flex items-center gap-3 px-4 sm:px-6 py-4 border-b border-[var(--bd)] shrink-0">
          ...
          <div className="flex-1 min-w-0">
            ...
            {ss.subject && (
              <span className="text-indigo-400 text-xs">{ss.subject}</span>
            )}
          </div>
          {/* Status pill */}
        ...
        <div className="flex-1 overflow-y-auto no-scrollbar px-4 sm:px-6 py-6">
          {tab === 'overview'   && <OverviewTab ss={ss} onRefresh={handleUploaded} />}
          {tab === 'flashcards' && <FlashcardsTab ss={ss} />}
          {tab === 'chat'       && <ChatTab ss={ss} />}
        </div>
      </main>
    </div>
  );
